...
void main() {
  test('roundtrip preserva transactionId (handoff exige readback exato)',
      () async {
    ...
    try {
      ...
      expect(
        await store.readPendingInstall(channel: 'beta'),
        isNull,
      );

      await store.clearPendingInstall(channel: 'stable');
      expect(
        await store.readPendingInstall(channel: 'stable'),
        isNull,
      );
    } finally {
