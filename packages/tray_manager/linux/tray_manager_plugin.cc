...
static FlMethodResponse* set_icon(TrayManagerPlugin* self, FlValue* args) {
  ...
  if (!indicator) {
    // app_indicator_new is deprecated since libappindicator 0.5.94 /
    // libayatana-appindicator (no replacement exists): silence -Werror
    // locally instead of downgrading the project's warning level.
    // Upstream: https://github.com/leanflutter/tray_manager/issues/67
