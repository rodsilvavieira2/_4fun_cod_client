...
#ifdef __linux__
constexpr const char* kPortalWindowSourceId = "portal:window";
constexpr const char* kPortalScreenSourceId = "portal:screen";
...
bool IsWaylandSession() {
  ...
  return wayland_display != nullptr && std::string(wayland_display).size() > 0;
}

bool IsPortalSourceId(const std::string& source_id) {
  return source_id == kPortalWindowSourceId ||
         source_id == kPortalScreenSourceId;
}

class PortalMediaSource : public MediaSource {
 public:
  PortalMediaSource(std::string id, std::string name, DesktopType type)
  ...
  portable::vector<unsigned char> thumbnail() const override { return {}; }
 ...
 protected:
  ~PortalMediaSource() override = default;
...
scoped_refptr<MediaSource> CreatePortalMediaSource(
    const std::string& source_id) {
  if (source_id == kPortalWindowSourceId) {
    return new RefCountedObject<PortalMediaSource>(
        kPortalWindowSourceId, "Selecionar janela...", DesktopType::kWindow);
  }
  return new RefCountedObject<PortalMediaSource>(
      kPortalScreenSourceId, "Selecionar display...", DesktopType::kScreen);
}
#endif
...
#ifdef __linux__
    // On GNOME/Wayland, RTCDesktopMediaListImpl crashes while constructing the
    // window media list on some libwebrtc builds. Avoid that path entirely and
    // expose a synthetic portal-backed source instead; the actual window is
    // chosen later by xdg-desktop-portal/PipeWire when capture starts.
    if (IsWaylandSession() && desktop_type == DesktopType::kWindow) {
      sources_.push_back(CreatePortalMediaSource(kPortalWindowSourceId));
      continue;
    }
#endif
